...
- Eval / Replay 已有 admin-only 确定性读模型和部分运行入口；reviewed trace fixture 目前只开放 intake 合同、repo/classpath manifest 覆盖视图、作者 template/schema、workbench catalog patch review 中的 `reviewedFixtureReadiness` 摘要、M5.85-43 的手动 candidate preview、M5.85-44 的 `/api/agent/observability/eval/workbench/trace-sets/{traceSetId}/reviewed-fixture-candidate-workbench` 自动候选预检、M5.85-45 的 `/api/agent/observability/eval/workbench/trace-sets/{traceSetId}/reviewed-fixture-human-review-package` 人工 Git review 准备包，以及 M5.85-47 的 `/api/agent/observability/eval/workbench/trace-sets/{traceSetId}/reviewed-fixture-human-review-gate` 人审字段/sha256 validate-only 门禁；这些入口只组合 redacted audit discovery、candidate preview、人工必填字段、复核清单、质量门预期和摘要校验。gate 通过时的 `readyForFixtureCommit=true` 仅表示“人工 Git 提交可以继续”，`runtimeFixtureCommitAllowed=false`、`createsFixtureFile=false`、`runtimeCatalogWrite=false` 仍是硬边界；CI blocking、LLM eval、Memory/RAG retrieval eval runtime 仍必须经过 reviewed trace evidence 和单独 release 审查。
