...
| 中文代码注释 | 分批推进 | 已完成安全入口、Tool/MCP/kube-manager 执行边界、Orchestrator / Graph / ReAct / Plan 编排链路、Memory / RAG / Eval / Observability / Audit 证据链，以及 Batch 5 的 DTO / store / config、support contract、query/path/body helper、分析目录 query helper；Eval reviewed trace fixture 已补 intake 合同、repo/classpath manifest、template/schema 只读视图，并已把 fixture readiness 汇入 workbench catalog patch review；M5.85-43 新增 reviewed fixture candidate preview，把 redacted replay / deterministic eval 摘要整理成人审/Git review 前的候选草稿和缺口，且不创建 fixture、不写 catalog、不嵌入 raw replay/report/fixture rows。 |
...
- Eval / Replay 已有 admin-only 确定性读模型和部分运行入口；reviewed trace fixture 目前只开放 intake 合同、repo/classpath manifest 覆盖视图、作者 template/schema、workbench catalog patch review 中的 `reviewedFixtureReadiness` 摘要，以及 M5.85-43 新增的 `/api/agent/observability/eval/workbench/trace-sets/{traceSetId}/reviewed-fixture-candidate` 候选预检；该预检只生成 `candidateFixtureDraft`、质量门摘要和剩余人审字段，`readyForFixtureCommit=false`，不接收上传、不写 catalog、不授予 CI/release 权力；CI blocking、LLM eval、Memory/RAG retrieval eval runtime 仍必须经过 reviewed trace evidence 和单独 release 审查。
...
# 开启后还会跑 SafeToolExecutor 真实 Agent 执行链和代表性 ToolCallback -> SafeToolExecutor 链路。
