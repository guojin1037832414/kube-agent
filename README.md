...
- Eval / Replay 已有 admin-only 确定性读模型和部分运行入口；reviewed trace fixture 目前只开放 intake 合同、repo/classpath manifest 覆盖视图、作者 template/schema、workbench catalog patch review 中的 `reviewedFixtureReadiness` 摘要、M5.85-43 的手动 candidate preview，以及 M5.85-44 新增的 `/api/agent/observability/eval/workbench/trace-sets/{traceSetId}/reviewed-fixture-candidate-workbench` 自动候选预检；该工作台只组合 redacted audit discovery 与 candidate preview，`readyForFixtureCommit=false`，不接收 caller traceId、不接收上传、不写 catalog、不授予 CI/release 权力；CI blocking、LLM eval、Memory/RAG retrieval eval runtime 仍必须经过 reviewed trace evidence 和单独 release 审查。
