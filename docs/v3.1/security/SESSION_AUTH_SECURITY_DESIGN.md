...
| 方案 | 描述 | 评估 |
|------|------|------|
| **A: X-Session-Id 仅作 conversation 标识** | Authorization 仍用 Bearer Token，X-Session-Id 仅标识 conversation | ✅ 推荐 — 职责清晰，Token 仍由 kube-manager 签发验证 |
...
后端处理:
  AuthTokenFilter  → 提取 Bearer Token → ThreadLocal
  (可选) SessionValidationFilter → 校验 X-Session-Id 是否存在于 ConversationStore
