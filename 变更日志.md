...
## [M5.85-39] - Catalog patch review fixture readiness

**Delivery**: Connected reviewed trace fixture manifest readiness into the eval workbench catalog patch review response.

**Changes**
- `AgentEvalWorkbenchCatalogPatchReviewService` now composes `AgentReviewedTraceFixtureManifestService` when available, while keeping the legacy constructor for focused tests and compatibility.
- `AgentEvalWorkbenchCatalogPatchReviewResponse` now exposes compact `reviewedFixtureReadiness` evidence: manifest status, fixture/catalog coverage counts, current trace-set fixture status, merge-before-fixture requirement, read-only endpoint links, and closed runtime flags.
- Added checklist and next-action guidance for missing reviewed fixture evidence before catalog patch merge, without changing `readyForGitReview` into runtime catalog write authority.
- Added service and controller regression coverage proving the response does not embed raw fixture rows, replay reports, raw audit secrets, or runtime write authority.
- Updated README, roadmap, mission memory, architecture/learning notes, and this changelog; `codex-memory/kube-agent/current` mirror update was attempted but blocked by local write permission on that directory.

**Verification**
- `mvn -q "-Dtest=AgentEvalWorkbenchCatalogPatchReviewServiceTest,ObservabilityControllerTest,ObservabilityControllerSecurityContractTest" test` passed.

**Security**
- This slice remains admin-only/read-only/review-only. It does not upload fixtures, accept caller trace IDs as evidence, mutate `eval-trace-sets.json`, execute eval/replay runtime, call Tool/MCP/LLM/RAG/kube-manager, write audit/memory, enable CI blocking, grant release authority, or touch Phase 2 NIM/HPC/Slurm/BCM scope.
