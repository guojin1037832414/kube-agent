...
> Phase 1 硬规则：新增 Tool 只能先走只读或证据读模型。任何真实 CREATE / UPDATE / DELETE / ACTION
> Tool 在一期合入前，必须证明已绑定 `SafeToolExecutor`、受保护参数过滤、可信 token/orgId、HITL、
> durable audit prewrite、幂等键、post-write readback、release gate、回滚/补偿说明和测试证据。
> 只有“用户确认了”不等于可以执行写操作。
...
- 读取租户时只能调用 `BaseTool#resolveOrganizationId(params)`；禁止从 `params.organizationId`、
  `params.orgId`、LLM 输出或前端字段中自行拼 kube-manager 路径。
...
public class {Resource}QueryTool extends BaseTool {
    ...
    // organizationId 是租户安全边界，只能来自认证链路绑定的 ThreadLocal。
    // LLM / Plan / 前端即使传入 organizationId，也会在 SafeToolExecutor/ProtectedToolParameterFilter 中被视为不可信。
...
### 6.2 创建类 Tool 模板（Phase 1 默认 HOLD）

> 注意：下面模板只说明写类 Tool 的代码形态，不代表可以直接开放真实写入。Phase 1 新增写类 Tool
> 必须先以 HOLD / dry-run / evidence-only 方式落地，真实执行必须等待 durable audit、幂等、HITL、
> release evidence 和 post-write readback 全部就绪。
...
@AtlasToolMapping(
    ...
    intentId    = "{intentId}",
    description = "{给LLM看的描述}",
    operationType = AtlasToolMapping.OperationType.CREATE // 写类 Tool 必须显式声明风险类型，不能保留 UNKNOWN
)
...
- [ ] Tool 没有从 `params.organizationId` / `params.orgId` / 请求体 / LLM 参数读取租户；租户只来自 `resolveOrganizationId(params)`
- [ ] 写类 Tool 显式声明 `operationType`，不能保留 UNKNOWN 风险元数据
- [ ] 写类 Tool 在 Phase 1 没有绕过 durable audit、idempotency、HITL、release gate、post-write readback 和 `SafeToolExecutor`
- [ ] NIM / HPC / Slurm / BCM 相关 Tool 属于 Phase 2 暂停域，不新增一期 runtime authority，也不导出到 MCP manifest
