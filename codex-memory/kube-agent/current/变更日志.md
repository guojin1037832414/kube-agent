...
## [M5.85-38] - Real Agent execution chain READ smoke

**Delivery**: Completed the real Agent execution-chain validation for the existing kube-manager `8100` READ smoke batch, moving beyond direct `tool.execute(...)` calls.

**Changes**
- Extended `KubeManagerReadOnlySmokeTest` with an opt-in `SafeToolExecutor` smoke that executes all six stable GET/READ/no-HITL Tool chains against the real local kube-manager backend.
- Added a representative opt-in `AtlasToolCallback -> SafeToolExecutor` smoke using `DashboardImageCountTool`, proving the Spring AI ToolCallback adapter still delegates through the unified safety boundary before touching kube-manager.
- Bound the smoke identity through both `UserPermissionContext.onLogin(...)` and ThreadLocal token/orgId, so `AUTHENTICATED` Tool visibility and kube-manager HTTP credentials mirror the real server-side login context.
- Added assertions for `SafeToolExecutionResult`, execution source, trace propagation, successful READ audit events, trusted organizationId, and protected parameter summaries for forged `organizationId/orgId/userId/token/HITL/audit/release/writeAllowed` fields.
- Kept the source guard unchanged: the only allowed POST remains `/api/login`; business chains remain GET-only and no Phase 2 Tool or write method is introduced.
- Updated README, route map, learning/architecture memory, current mission memory, recovery status, and this changelog.

**Verification**
- `mvn -q "-Dtest=KubeManagerReadOnlySmokeTest" test` passed with real smoke disabled by default.
- Login-backed opt-in smoke against `http://localhost:8100` passed with the local test account and resolved trusted orgId, covering direct Tool calls, all six `SafeToolExecutor` READ chains, and the representative ToolCallback chain.
- `mvn -q test` passed.
- `mvn -q "-DskipTests" package` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.
- Sensitive scan found only placeholder values such as `<当前密码>` and `password=xxx`, not the real test password or token.
- Branch topology check confirmed remote heads are still only `develop` and `master`.

**Security**
- This slice changes only opt-in test coverage and documentation; it does not change production Tool behavior or open new Controller APIs.
- No kube-manager write, sensitive-read smoke target, MCP `tools/call`, HITL marker creation, audit/memory write, retrieval/vector runtime, CI blocking, release authority, dependency upgrade, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.
- Test password and temporary token stayed in process memory/environment only and were not written to source, docs, memory, logs, or Git.

## [M5.85-37] - Full validation hardening

**Delivery**: Completed the broader validation pass requested after the kube-manager READ smoke batch and fixed a Spring context wiring issue exposed only by full `mvn test`.

**Changes**
- Added explicit constructor injection to `AgentReviewedTraceFixtureManifestService` so Spring can choose the production constructor when a package-private test constructor also exists.
- Verified that the failure was a full-context wiring issue in the reviewed fixture manifest service, not a regression in the kube-manager READ smoke batch.
- Re-ran the previously failing full-context representatives `RuleMatcherTest` and `ToolRegistryPermissionTest`.
- Re-ran full Maven tests, real kube-manager smoke, package build, format check, sensitive scan, and branch/worktree checks.

**Verification**
- `mvn -q "-Dtest=RuleMatcherTest,ToolRegistryPermissionTest" test` passed.
- `mvn -q test` passed.
- Login-backed opt-in smoke against `http://localhost:8100` passed with the local test account and resolved trusted orgId, covering all six READ Tool chains.
- `mvn -q "-DskipTests" package` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.
- Sensitive scan found only placeholder values such as `<当前密码>` and `password=xxx`, not the real test password or token.

**Security**
- This slice opens no runtime authority. It only fixes Spring DI metadata and runs validation.
- No kube-manager write, sensitive-read smoke target, MCP `tools/call`, HITL marker, audit/memory write, retrieval/vector runtime, CI blocking, release authority, dependency upgrade, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-36] - kube-manager READ smoke batch

**Delivery**: Completed a broader kube-manager `8100` READ smoke batch, expanding the opt-in real backend smoke to six stable GET/READ/no-HITL Tool chains.

**Changes**
- Extended `KubeManagerReadOnlySmokeTest` metadata guard to cover `NodeQueryTool`, `NodeRemainingResourceTool`, `DashboardDeploymentCountTool`, `DashboardImageCountTool`, `DashboardEasyFlowCountTool`, and `DashboardEasyFlowTool`.
- Added assertions that every smoke target remains `GET`, `READ`, no-confirmation, explicitly permissioned, and fixed to a reviewed kube-manager endpoint.
- Updated the opt-in real smoke to execute `/node`, `/node/remaining`, `/dashboard/deployment/count`, `/dashboard/image/count`, `/dashboard/easy-flow/count`, and `/dashboard/easy-flow` after resolving trusted orgId through the login-backed identity path.
- Kept forged `organizationId`, `orgId`, `token`, pagination, and keyword fields in Tool parameters so the smoke continues to prove Tool execution uses trusted `UserPermissionContext`, not caller-supplied control fields.
- Documented that `ClusterOverviewTool -> /dashboard/resources` is deferred from the stable smoke batch because it timed out under the JUnit smoke read timeout even though a standalone probe returned 200.
- Updated README, learning guide, route map, architecture learning map, current mission memory, recovery status, and this changelog.

**Verification**
- `mvn -q "-Dtest=KubeManagerReadOnlySmokeTest" test` passed.
- Login-backed opt-in smoke against `http://localhost:8100` passed with the local test account and resolved trusted orgId, covering all six READ Tool chains.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice changes only test/documentation coverage for low-risk READ smoke; it does not change production Tool behavior or open new Controller APIs.
- The only POST allowed in the smoke remains auth bootstrap `/api/login`; business chains remain GET-only.
- It does not add kube-manager writes, user/log/permission/order/quota sensitive-read smoke targets, MCP `tools/call`, HITL markers, audit/memory writes, retrieval/vector runtime, CI blocking, release authority, dependency upgrades, or Phase 2 NIM/HPC/Slurm/BCM authority.

## [M5.85-35] - kube-manager second READ smoke

**Delivery**: Expanded the opt-in kube-manager `8100` READ smoke from one real Tool chain to two real GET/READ/no-HITL Tool chains.

**Changes**
- Extended `KubeManagerReadOnlySmokeTest` so its always-on metadata guard covers both `NodeQueryTool` and `DashboardDeploymentCountTool`.
- Added assertions that the second smoke target remains `GET`, `READ`, no-confirmation, `ToolPermission.PUBLIC`, and fixed to `/api/{orgId}/dashboard/deployment/count`.
- Updated the opt-in real smoke to execute `NodeQueryTool -> /api/{orgId}/node` and `DashboardDeploymentCountTool -> /api/{orgId}/dashboard/deployment/count` after resolving the trusted orgId from the login-backed identity path.
- Kept forged `organizationId`, `orgId`, and `token` parameters in both Tool calls so the test continues to prove Tool execution uses the trusted `UserPermissionContext`, not caller-supplied control fields.
- Updated README, route memory, current mission memory, recovery status, and this changelog.

**Verification**
- `mvn -q "-Dtest=KubeManagerReadOnlySmokeTest" test` passed.
- Login-backed opt-in smoke against `http://localhost:8100` passed with the local test account and resolved trusted orgId, covering both READ Tool chains.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.
...
- This slice changes only test/documentation coverage for low-risk READ smoke; it does not change production Tool behavior or open new Controller APIs.
- The only POST allowed in the smoke remains auth bootstrap `/api/login`; business chains remain GET-only.
- It does not add kube-manager writes, sensitive-read smoke targets, user-list smoke targets, MCP `tools/call`, HITL markers, audit/memory writes, retrieval/vector runtime, CI blocking, release authority, dependency upgrades, or Phase 2 NIM/HPC/Slurm/BCM authority.

## [M5.85-34] - Reviewed trace fixture template/schema

**Delivery**: Added an admin-only authoring template/schema endpoint so reviewed redacted trace fixture files can be prepared consistently before any real fixture JSON is committed.

**Changes**
- Added `AgentReviewedTraceFixtureTemplateService` and `AgentReviewedTraceFixtureTemplateResponse`.
- Added `GET /api/agent/observability/eval/reviewed-trace-fixture-template` to `ObservabilityController`.
- The response publishes schema `agent-reviewed-trace-fixture-template.v1`, JSON Schema, placeholder example skeleton, structured proof blocks, trace-set-specific suggested filenames, file naming rules, authoring workflow, forbidden shortcuts, endpoint navigation, safety proof, and privacy proof.
- Added `src/main/resources/observability/reviewed-trace-fixtures/README.md` to document that the scanned fixture directory must contain only real human-reviewed fixture JSON, never templates or placeholders.
- Linked the template endpoint from the existing fixture intake contract and fixture manifest endpoint maps.
- Added service tests, Controller admin/forbidden tests, Spring Security MockMvc coverage, Controller source-contract coverage, and Chinese teaching-comment source markers.

**Verification**
- `mvn -q "-Dtest=AgentReviewedTraceFixtureTemplateServiceTest,Batch4ReviewedTraceFixtureTemplateChineseCommentContractTest,ObservabilityControllerSecurityContractTest,ObservabilityControllerTest,AgentSecurityConfigWebMvcTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- The new endpoint is admin-only, read-only, template-only, and schema-only.
- It does not create fixture files, accept placeholder trace IDs, accept uploads, accept caller trace IDs, mutate catalogs, write `eval-trace-sets.json`, execute eval/replay, call Tool/MCP/LLM/RAG/kube-manager, create HITL markers, write audit/memory, enable CI blocking, grant release authority, upgrade dependencies, or touch Phase 2 NIM/HPC/Slurm/BCM authority.

## [M5.85-33] - Reviewed trace fixture manifest

**Delivery**: Added an admin-only repo/classpath manifest for reviewed redacted trace fixtures so Phase 1 can see fixture file coverage before any catalog patch review.

**Changes**
- Added `AgentReviewedTraceFixtureManifestService` to scan `classpath*:observability/reviewed-trace-fixtures/*.json` and compare discovered fixture files with the trace-set catalog.
- Added `AgentReviewedTraceFixtureManifestResponse` with schema `agent-reviewed-trace-fixture-manifest.v1`, fixture rows, trace-set coverage, required fixture fields, forbidden shortcuts, endpoint navigation, safety proof, and privacy proof.
- Added `GET /api/agent/observability/eval/reviewed-trace-fixture-manifest` to `ObservabilityController`.
- Added service tests, Controller admin/forbidden tests, Spring Security MockMvc coverage, Controller source-contract coverage, and Chinese teaching-comment source markers.

**Verification**
- `mvn -q "-Dtest=AgentReviewedTraceFixtureManifestServiceTest,Batch4ReviewedTraceFixtureManifestChineseCommentContractTest,ObservabilityControllerSecurityContractTest,ObservabilityControllerTest,AgentSecurityConfigWebMvcTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- The new endpoint is admin-only, read-only, manifest-only, and classpath-scan-only.
- It does not accept fixture uploads, caller trace IDs, request bodies, runtime catalog writes, `eval-trace-sets.json` mutation, eval/replay execution, Tool/MCP/LLM/RAG/kube-manager calls, HITL marker creation, audit/memory writes, CI blocking, release authority, dependency upgrades, or Phase 2 NIM/HPC/Slurm/BCM authority.
- Redacted stale plaintext test-account password examples from the legacy recovery guide; future real smoke credentials must stay in process environment/properties only.

## [M5.85-32] - kube-manager login-backed READ smoke

**Delivery**: Upgraded the opt-in kube-manager 8100 READ smoke from token/orgId-only manual setup to a repeatable login-backed path that keeps credentials in the test process and then runs the same read-only NodeQueryTool chain.

**Changes**
- Extended `KubeManagerReadOnlySmokeTest` with username/password identity mode through `ATLAS_KUBE_MANAGER_SMOKE_USERNAME` / `ATLAS_KUBE_MANAGER_SMOKE_PASSWORD` or matching system properties.
- Kept token/orgId direct mode, but now requires them to be provided as a pair so old tokens and hand-written orgIds cannot be mixed into an unauditable smoke identity.
- Reused production `KubeManagerHttpClient.resolveOrgId(username, token)` after login so the trusted orgId comes from `/api/{orgId}/user`, not from the kube-manager login form `organizationId`.
- Updated the source guard: the only allowed POST in this smoke is auth bootstrap `/api/login`; the business chain remains GET/READ/no-HITL through `NodeQueryTool -> /api/{orgId}/node`.
- Updated README, route map, architecture learning map, current mission memory, recovery status, and this changelog.

**Verification**
- `mvn -q "-Dtest=KubeManagerReadOnlySmokeTest" test` passed with the real external path skipped by default.
- Login-backed opt-in run against `http://localhost:8100` passed with username/password supplied through environment variables; trusted orgId was resolved through kube-manager user lookup and the node query READ smoke succeeded.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- Test-only and documentation-only runtime impact; production login/session/Tool execution behavior did not change.
- Passwords and tokens are read only from process properties/environment and are not written to source, docs, recovery memory, git, or failure messages.
- No kube-manager business write, MCP `tools/call`, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, CI blocking, release authority, dependency upgrade, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-30] - Reviewed trace fixture intake contract

**Delivery**: Added an admin-only reviewed redacted trace fixture intake contract so Phase 1 has an explicit specification for what a trace fixture must contain before human/Git review and later catalog promotion.

**Changes**
- Added `AgentReviewedTraceFixtureIntakeContractService` and `AgentReviewedTraceFixtureIntakeContractResponse`.
- Added `GET /api/agent/observability/eval/reviewed-trace-fixture-intake-contract` to `ObservabilityController`.
- The response publishes required fixture fields, review workflow, quality gates, per trace-set readiness, forbidden shortcuts, endpoint navigation, safety proof, and privacy proof.
- Added service tests, Controller admin/forbidden tests, Spring Security MockMvc coverage, Controller source-contract coverage, and Chinese teaching-comment source markers.

**Verification**
- `mvn -q "-Dtest=AgentReviewedTraceFixtureIntakeContractServiceTest,Batch4ReviewedTraceFixtureIntakeChineseCommentContractTest,ObservabilityControllerSecurityContractTest,ObservabilityControllerTest,AgentSecurityConfigWebMvcTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- The new endpoint is admin-only, read-only, contract-only, and intake-spec-only.
- It does not accept fixture uploads, caller trace IDs, request bodies, runtime catalog writes, `eval-trace-sets.json` mutation, eval/replay execution, Tool/MCP/LLM/RAG/kube-manager calls, HITL marker creation, audit/memory writes, CI blocking, release authority, dependency upgrades, or Phase 2 NIM/HPC/Slurm/BCM authority.

## [M5.85-29] - Learning guide technology knowledge cards

**Delivery**: Further enriched the kube-agent top-tier Agent learning guide with detailed technology knowledge cards, source-reading routes, official documentation study order, adoption gates, and practice tasks.

**Changes**
- Added `19. 技术知识点学习卡片` to `docs/learning/以kube-agent为例的顶级Agent开发学习指南.md`, covering Java/Maven/Spring Boot, Spring Security, LLM/Tool Calling/structured output, Graph/ReAct/Plan/Multi-Agent, Tool governance, kube-manager HTTP outlet, HITL/Audit/Release Gate, Memory/RAG/Embedding/VectorStore, Observability/Tracing/Eval/Replay, MCP/A2A, testing, quality, and supply chain.
- Added `20. 源码精读路线` so learners can trace a real chat request from Security through Orchestrator, Graph/ReAct/Plan, SafeToolExecutor, kube-manager outlet, audit, replay, eval, memory/RAG, MCP, and tests.
- Added `21. 官方文档精读顺序`, `22. 新技术采纳规则`, and `23. 学习练习任务库` to turn official docs into project practice and keep new technology adoption behind compatibility, safety, eval, release, and recovery-memory gates.
- Rechecked official/reference links and replaced moved Spring AI / OpenAI URLs with currently reachable official paths.

**Verification**
- Official/reference link sampling passed after updating moved links.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- Documentation-only slice; no Java runtime behavior changed.
- Explicitly preserved the rule that learning latest Java/Spring/Spring AI/MCP/A2A/RAG/Eval technology is not the same as enabling runtime authority.
- No Tool/MCP/kube-manager write, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, runtime catalog write, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-28] - Eval trace promotion workflow Chinese comments

**Delivery**: Added Chinese teaching comments to the Eval trace promotion workflow and workbench wrapper so the catalog-promotion evidence path is easier to learn and review.

**Changes**
- Added Chinese teaching comments to `AgentEvalTraceSetPromotionWorkflowService`, `AgentEvalTraceSetPromotionWorkflowArtifact`, `AgentEvalTraceSetPromotionWorkflowRequest`, `AgentEvalWorkbenchPromotionWorkflowService`, and `AgentEvalWorkbenchPromotionWorkflowResponse`.
- Added Chinese teaching comments to promotion workflow tests covering read-only workflow composition, empty-candidate fail-closed behavior, and workbench wrapper projection.
- Added `Batch4EvalTracePromotionWorkflowChineseCommentContractTest` to preserve markers around workflow-only/proposal-only orchestration, human Git review, `readyForGitReview` not being release authority, UI steps as read-model navigation, and closed runtime authorities.

**Verification**
- `mvn -q "-Dtest=Batch4EvalTracePromotionWorkflowChineseCommentContractTest,AgentEvalTraceSetPromotionWorkflowServiceTest,AgentEvalWorkbenchPromotionWorkflowServiceTest,AgentEvalWorkbenchCapabilitiesServiceTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- Comments plus source-contract coverage only; no production behavior changed.
- Promotion workflow and workbench wrapper remain read-model / workflow-only / proposal-only layers; `readyForGitReview` is human Git review evidence only, not catalog write authority or release authority.
- No Tool/MCP/kube-manager write, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, runtime catalog write, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-27] - Top-tier Agent learning guide technical deep dive

**Delivery**: Expanded the kube-agent learning guide again into a deeper technical study manual with project-specific explanations and official learning links.

**Changes**
- Added a new technical deep-dive index to `docs/learning/以kube-agent为例的顶级Agent开发学习指南.md` covering Java/Maven/Spring control plane, LLM integration, Tool Calling, structured output, Graph/ReAct/Plan/Multi-Agent orchestration, Tool governance, HITL/guardrails/audit/release gates, Memory/RAG/VectorStore/GraphRAG, Eval/Replay/Observability, MCP resources/prompts/tools, Agent security, and testing/quality gates.
- Added a projectized "from beginner to expert" learning route that maps each study phase to kube-agent source-reading and slice-delivery practice.
- Added a consolidated official documentation navigation table for Java, Maven, Spring, Spring AI, OpenAI Agents, MCP, Kubernetes, OpenTelemetry, RAG/embedding, OWASP/NIST security, and testing/quality tooling.

**Verification**
- Official/reference link sampling passed with HTTP 200 responses.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- Documentation-only slice; no Java runtime behavior changed.
- No Tool/MCP/kube-manager write, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-26] - Eval trace curation Chinese comments

**Delivery**: Added Chinese teaching comments to the Eval trace evidence / curation chain so the release-gate evidence workflow is easier to learn and review.

**Changes**
- Added Chinese teaching comments to `AgentEvalTraceSetCatalogService`, `AgentEvalTraceSetDefinition`, `AgentEvalTraceSetCandidateDiscoveryService`, `AgentEvalTraceSetCandidate`, `AgentEvalTraceSetCandidateDiscoveryResponse`, `AgentEvalTraceSetCurationReviewArtifact`, and `AgentEvalTraceSetCatalogPatchProposalArtifact`.
- Clarified that trace set catalog, candidate discovery, curation review, and JSON Patch proposal are read-model / review-only / artifact-only layers.
- Added `Batch4EvalTraceCurationChineseCommentContractTest` to preserve teaching markers around redacted replay evidence anchors, review-only curation, non-mutating catalog patches, and closed runtime authorities.

**Verification**
- `mvn -q "-Dtest=Batch4EvalTraceCurationChineseCommentContractTest,AgentEvalTraceSetCatalogServiceTest,AgentEvalTraceSetCandidateDiscoveryServiceTest,AgentMemoryRagReviewedTraceEvidenceManifestServiceTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- Comments plus source-contract coverage only; no production behavior changed.
- Eval traceIds remain redacted replay evidence anchors, not identity, tenant, Tool params, HITL token, audit receipt, release authority, retrieval runtime, or CI blocking authority.
- No Tool/MCP/kube-manager write, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-25] - Expanded top-tier Agent learning guide

**Delivery**: Enriched the project learning guide so it can serve as a structured curriculum for learning top-tier Agent backend engineering through kube-agent.

**Changes**
- Expanded `docs/learning/以kube-agent为例的顶级Agent开发学习指南.md` with a technology stack learning map, official/reference learning links, core concept explanations, code-reading routes, common pitfalls, review checklists, exercises, and phased learning tasks.
- Covered Spring Boot, Spring Security, Spring AI, OpenAI tool/agent concepts, Spring AI Alibaba Graph/ReAct, MCP, kube-manager/Kubernetes API thinking, Resilience4j, Micrometer/OpenTelemetry, Memory/RAG, ONNX/DJL, Caffeine, OWASP LLM Top 10, NIST AI RMF, JUnit/AssertJ/Mockito, ArchUnit/Testcontainers, JaCoCo/SpotBugs/CycloneDX.
- Added project-specific explanations for identity/tenant/session/conversation separation, Tool calling as application-side execution, prompt injection boundaries, HITL, write release gates, RAG source custody, Eval/Replay/Observability, MCP governance, frontend/backend responsibility, and slice workflow.
...
## [M5.85-24] - Batch 5 support test Chinese comments

**Delivery**: Continued Batch 5 by documenting config/store/intent support tests as learning assets and preserving those markers with a source-contract test.

**Changes**
- Added Chinese teaching comments to `TokenPropagatingTaskDecoratorTest`, `ConversationStoreTest`, `IntentArbiterTest`, `EmbeddingMatcherMockTest`, and `RuleMatcherTest`.
- Clarified async token/orgId propagation, ThreadLocal cleanup, conversation owner filtering, intent arbitration, embedding degradation, and rule matching as explicit teaching contracts.
- Added `Batch5SupportTestChineseCommentContractTest` to preserve test-level markers around service-trusted context, conversation ownership, routing evidence, fail-soft embedding behavior, and rule-match boundaries.

**Verification**
- `mvn -q "-Dtest=Batch5SupportTestChineseCommentContractTest,TokenPropagatingTaskDecoratorTest,ConversationStoreTest,IntentArbiterTest,EmbeddingMatcherMockTest" test` passed.
- `mvn -q "-Dtest=RuleMatcherTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice changes test comments and adds a source-contract test only.
- `conversationId` remains a resource locator, not authorization; intent scores, `crossBoost`, embedding similarity, keyword hits, and regex hits remain routing evidence only.
- No MCP `tools/call`, kube-manager write, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-23] - Batch 5 tool core test Chinese comments

**Delivery**: Continued Batch 5 by documenting key Tool core tests as teaching assets, so tests explain the safety contracts they protect.

**Changes**
- Added Chinese teaching comments to Tool core tests covering prompt visibility, permission-based Tool visibility, parameter normalization, input schema, protected parameter filtering, shared filter usage, organizationId governance, and legacy ToolCallback safe execution.
- Added `Batch5ToolCoreTestChineseCommentContractTest` to preserve those test-level learning markers.
- Clarified that these tests do not call real LLMs, kube-manager, MCP, durable audit/memory, HITL, or Phase 2 runtime abilities.

**Verification**
- `mvn -q "-Dtest=Batch5ToolCoreTestChineseCommentContractTest,ToolParameterNormalizerTest,ToolInputSchemaBuilderTest,ProtectedToolParameterFilterTest,ProtectedToolParameterFilterUsageContractTest,BaseToolOrganizationIdGovernanceTest,AtlasToolCallbackSafeExecutorTest" test` passed.
- `mvn -q "-Dtest=ToolRegistryPermissionTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice changes test comments and adds a source-contract test only.
- ToolRegistry visibility remains Prompt/catalog visibility, not final execution authorization.
- No MCP `tools/call`, kube-manager write, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-22] - Batch 5 intent routing Chinese comments

**Delivery**: Continued Batch 5 by documenting the intent routing chain that turns natural language into candidate intents without granting runtime authority.

**Changes**
- Added Chinese teaching comments to `IntentRouter`, `IntentArbiter`, `RuleMatcher`, `EmbeddingMatcher`, `IntentsLoader`, and `L3IntentClassifier`.
- Clarified that L1/L2/L3/L4 hits, high-confidence short-circuiting, `crossBoost`, prompt snapshots, keyword/regex matches, and intents.yml entries are routing evidence only.
- Documented that intent routing must not register Tools, skip `SafeToolExecutor`, create HITL markers, write audit/memory, call MCP, call kube-manager, or infer token/orgId/userId/write/release control fields.
- Added `Batch5IntentRoutingChineseCommentContractTest` to preserve these learning markers.

**Verification**
- `mvn -q "-Dtest=Batch5IntentRoutingChineseCommentContractTest,Batch5ChineseCommentContractTest,IntentArbiterTest,RuleMatcherTest,EmbeddingMatcherMockTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice is comments plus source-contract coverage only; it changes no runtime routing behavior.
- Intent matches remain candidate routing signals, not Tool execution authorization.
- No MCP `tools/call`, kube-manager write, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-21] - Batch 5 core adapter Chinese comments

**Delivery**: Continued the Batch 5 Chinese teaching-comment rollout by documenting the Tool core adapter layer that connects Spring AI, LLM tool JSON, Tool schema, default values, Tool results, and SafeToolExecutor.

**Changes**
- Added Chinese teaching comments to `AtlasTool`, `AtlasToolCallback`, `ToolInputSchemaBuilder`, `AtlasToolResultConverter`, `AtlasToolResult`, `AtlasToolContext`, and `DefaultValueAspect`.
- Clarified that LLM tool JSON, ToolContext, inputSchema, defaults, and Tool result maps are candidate/display/support contracts, not runtime authorization sources.
- Removed misleading old comment text in the touched areas and explained why these support classes must not trust `token`, `orgId`, `userId`, HITL, audit, release, or write-control fields from caller params.
- Added `Batch5CoreAdapterChineseCommentContractTest` to preserve the new Chinese learning markers for the core adapter support layer.

**Verification**
- `mvn -q "-Dtest=Batch5CoreAdapterChineseCommentContractTest,Batch5ChineseCommentContractTest,Batch5SupportContractBehaviorTest,ToolInputSchemaBuilderTest,AtlasToolCallbackSafeExecutorTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice is comments plus source-contract coverage only; it changes no runtime execution behavior.
- `SafeToolExecutor` remains the real Tool execution boundary.
- No MCP `tools/call`, kube-manager write, HITL marker creation, audit/memory write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-20] - kube-manager READ 8100 smoke harness

**Delivery**: Added a safe, opt-in kube-manager `8100` READ smoke harness for real local query-path validation without making normal tests depend on external services.

**Changes**
- Added `KubeManagerReadOnlySmokeTest` under `src/test/java/com/atlas/e2e`.
- The test always verifies the smoke target remains `NodeQueryTool` with `GET`, `READ`, no HITL confirmation, and `/api/{orgId}/node`.
- Added a source-level guard to prevent the smoke harness from being expanded to POST/PUT/PATCH/DELETE or Phase 2 NIM/HPC/Slurm/BCM Tool bindings.
- Added an opt-in real smoke path gated by `atlas.kube-manager.smoke.enabled=true` or `ATLAS_KUBE_MANAGER_SMOKE_ENABLED=true`.
- The opt-in path requires current user `token` and trusted `orgId`, binds them through `UserPermissionContext`, initializes `KubeManagerHttpClient`, and executes `NodeQueryTool` with forged control-plane params to prove the Tool uses server context.
- Documented the smoke command in README and updated roadmap/recovery docs.

**Verification**
- `Test-NetConnection localhost -Port 8100` reported `TcpTestSucceeded=False`, so the real external smoke was not executed in this environment.
- `mvn -q "-Dtest=KubeManagerReadOnlySmokeTest,KubeManagerHttpClientUrlContractTest,KubeManagerHttpClientTracePropagationTest,VirtualMachineReadToolHttpContractTest" test` passed; the real 8100 case was skipped by JUnit assumptions because it was not explicitly enabled.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed.

**Security**
- This slice opens no new runtime authority. It adds a test-only, explicit opt-in READ smoke path.
- Normal tests never call kube-manager `8100`; real smoke requires caller-provided current user token and orgId.
- The harness is pinned to `NodeQueryTool` GET/READ/no-confirmation and does not call write methods, MCP runtime, HITL, audit/memory writes, retrieval/vector runtime, A2A handoff, or Phase 2 NIM/HPC/Slurm/BCM capabilities.

## [M5.85-19] - Supervisor Graph SSE final content dedupe

**Delivery**: Continued Orchestrator hardening by making main `supervisorGraph` final display content consistent across direct answer, Tool display, delegate, and ReAct paths.

**Changes**
- Added `emitSupervisorDisplayContent` and `registerSupervisorDisplayContent` to `AtlasOrchestrator` so one run can de-duplicate final SSE `content` text across Graph nodes.
- Made ReAct `content` events register their final answer before emitting, preventing the later `react_node_result` / `answer` fallback from duplicating the same front-end bubble.
- Routed main `supervisorGraph` Tool display, Tool fail-closed `answer`, delegate display, direct-answer `answer`, and ReAct fallback display through the same de-duplication helper.
- Added direct `direct_answer` SSE display handling so AtlasBrain ordinary answers are visible through `/api/agent/chat/stream`, not only through thinking events.
- Added Chinese teaching comments explaining that the SSE display de-duplication layer is not Tool success, HITL confirmation, audit receipt, release evidence, or write authority.
- Extended `AtlasOrchestratorJsonTest` with reflection tests for final content de-duplication, whitespace normalization, distinct message preservation, and placeholder rejection.
- Extended `Batch3ChineseCommentContractTest` to protect `direct_answer` and de-duplication teaching markers.

**Verification**
- `mvn -q "-Dtest=AtlasOrchestratorJsonTest,Batch3ChineseCommentContractTest,ToolResultMergeNodeTest,GraphToolCallSafetyGuardTest,GraphReActNodeSafetyGuardTest,GraphExecuteNodeSafetyGuardTest,SupervisorGraphReactRoutingTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice only changes SSE display projection and duplicate suppression; it does not execute Tools, call LLMs, call kube-manager, create HITL markers, write audit/memory, or grant permissions.
- De-duplicated content remains natural-language display text only and cannot be treated as Tool success, HITL confirmation, audit receipt, release-gate evidence, or write completion.
- No MCP `tools/call`, kube-manager write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, audit/memory write, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-18] - Delegate SSE display contract

**Delivery**: Continued Orchestrator hardening by making main `supervisorGraph` delegate results and delegate fail-closed answers visible through SSE.

**Changes**
- Added `delegateDisplayContent` to `AtlasOrchestrator` to select professional Agent output or fallback `answer` from delegate node state.
- Emitted delegate `content` SSE events when the `delegate` node produces user-visible output.
- Added Chinese teaching comments explaining delegate SSE projection is display-only and not Tool success, HITL, audit, release, or write evidence.
- Extended `AtlasOrchestratorJsonTest` with reflection tests for delegate result priority, delegate fail-closed answer fallback, and empty placeholder filtering.
- Extended `Batch3ChineseCommentContractTest` to protect delegate/fail-closed teaching markers in the orchestrator.
...
**Security**
- This slice only makes existing delegate state visible to SSE clients; it does not execute Tools, call LLMs, call kube-manager, create HITL markers, write audit/memory, or grant permissions.
- Delegate content remains display text only; real Tool calls inside Spring AI agent subgraphs still must pass ToolCallback / SafeToolExecutor.
- No MCP `tools/call`, kube-manager write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, audit/memory write, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-17] - Graph merge_result display priority

**Delivery**: Continued Orchestrator hardening by fixing old `atlasGraph` result merge semantics so user-visible answers and safety-stop reasons are not lost before SSE output.

**Changes**
- Updated `ToolResultMergeNode` to merge display results in this order: existing `final_answer`, `react_node_result`, professional Agent results, generic `answer`, `supervisor_result`, then explicit fallback error.
- Added Chinese teaching comments explaining that `merge_result` is display projection only, not Tool success, HITL, audit, release, or write evidence.
- Added `ToolResultMergeNodeTest` covering direct-answer preservation, ReAct result preference, fail-closed answer visibility, and explicit fallback error.
- Extended `Batch3ChineseCommentContractTest` to protect `ToolResultMergeNode` learning markers.

**Verification**
- `mvn -q "-Dtest=ToolResultMergeNodeTest,Batch3ChineseCommentContractTest,GraphToolCallSafetyGuardTest,GraphReActNodeSafetyGuardTest,GraphExecuteNodeSafetyGuardTest,SupervisorGraphReactRoutingTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice changes only result projection for old `atlasGraph`; it does not execute Tools, call LLMs, call kube-manager, create HITL markers, write audit/memory, or grant permissions.
- `final_answer` remains SSE display text only and cannot be treated as Tool success or release authority.
- No MCP `tools/call`, kube-manager write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, audit/memory write, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-16] - Graph execute_node trusted org guard

**Delivery**: Continued Orchestrator hardening by adding an explicit trusted-organization guard to Graph `execute_node` before Plan candidates can enter `SafeToolExecutor`.

**Changes**
- Updated `AtlasGraphConfig.buildExecuteNode` so single-step READ Plan execution first resolves orgId from Graph State, then from trusted `UserPermissionContext` ThreadLocal.
- Added fail-closed `EXECUTE_TRUSTED_ORG_MISSING` state before `SafeToolExecutionRequest` is created when no trusted orgId exists.
- Ensured `SafeToolExecutionRequest` receives the resolved trusted `orgId`, not a value derived from `PlanStep.parameters`.
- Added Chinese teaching comments explaining that Plan automatic execution candidates still need a tenant boundary even for READ steps.
- Added `GraphExecuteNodeSafetyGuardTest` to lock orgId recovery, fail-closed ordering, and request construction semantics.

**Verification**
- `mvn -q "-Dtest=GraphExecuteNodeSafetyGuardTest,M42PlanExecuteSafetyContractTest,SafeToolExecutorTest,ProtectedToolParameterFilterUsageContractTest,SupervisorGraphReactRoutingTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice narrows Plan automatic execution; it does not expand the existing single-step READ-only gate.
- PlanStep parameters remain untrusted business input and cannot provide orgId, token, userId, conversationId, HITL, audit, release, or write authority.
- No MCP `tools/call`, kube-manager write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, audit/memory write, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-15] - Graph ReAct node safety guard

**Delivery**: Continued Orchestrator hardening by aligning Graph `react_node` with the same fail-closed context model as Graph `tool_call`.

**Changes**
- Updated `AtlasGraphConfig.buildReActNode` to read server-owned `traceId` from Graph State and pass it into ReAct `initialParams`.
- Added a trusted orgId guard before `ReActEngine.runWithEvents`; when Graph State and server ThreadLocal both lack orgId, `react_node` now stops before calling LLM, Tool, or kube-manager.
- Added `failClosedGraphReActUpdates` to write user-visible `answer`, `react_node_result`, `tool_error_code`, `execute_result.executed=false`, source, and traceId.
- Added Chinese teaching comments explaining that ReAct `initialParams` are server context, while each LLM `Action.params` remains untrusted business input.
- Added `GraphReActNodeSafetyGuardTest` to protect trace injection, orgId fail-closed ordering, and non-executed Graph update semantics.

**Verification**
- `mvn -q "-Dtest=GraphReActNodeSafetyGuardTest,M523TracePropagationContractTest,ReActEngineParamMergeTest,ReActEngineMultiStepE2ETest,SupervisorGraphReactRoutingTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- This slice does not grant new runtime authority; it prevents ReAct from starting without a trusted tenant boundary.
- `traceId` is propagated for observability only and remains a protected control-plane field, not a Tool business parameter.
- No MCP `tools/call`, kube-manager write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, audit/memory write, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-14] - Graph tool_call safety guard

**Delivery**: Completed the first Orchestrator hardening slice by adding a fail-closed guard before Graph `tool_call` creates `SafeToolExecutionRequest`.

**Changes**
- Added `guardGraphToolCallCandidate` in `AtlasGraphConfig` so Graph `tool_call` stops before `SafeToolExecutor` when the candidate has no Tool target, no trusted orgId, or protected control fields in parameters.
- Added `failClosedGraphToolCallUpdates` to write a structured non-executed state with `answer`, `tool_error_code`, `execute_result.executed=false`, source, and traceId.
- Allowed Graph `tool_call` to recover orgId from trusted server ThreadLocal when Graph State is blank, while still fail-closing if no trusted orgId exists.
- Updated `AtlasOrchestrator` SSE handling so both the main `supervisorGraph` stream and experimental `/chat/graph` stream display Graph guard / execute-node safety-stop reasons even when no `tool_result` exists.
- Adjusted `PlanEngine` comments to avoid brittle source-contract false positives while preserving the same safety teaching.
- Added `GraphToolCallSafetyGuardTest` with Chinese teaching comments covering missing target, missing orgId, protected parameter rejection, and normal business-parameter pass-through.

**Verification**
- `mvn -q "-Dtest=GraphToolCallSafetyGuardTest,SupervisorGraphReactRoutingTest,M513HitlFailClosedContractTest,M42PlanExecuteSafetyContractTest,ProtectedToolParameterFilterUsageContractTest" test` passed.

**Security**
- This slice does not grant new runtime authority; it narrows Graph `tool_call` before the unified executor.
- `SafeToolExecutor` remains the final Tool execution boundary; the new Graph guard is an earlier evidence-producing fail-closed layer.
- Protected fields such as token, orgId, userId, conversationId, traceId, HITL confirmation, audit receipt, release decision, and write flags cannot pass through Graph `tool_call` parameters as authority.
- No MCP `tools/call`, kube-manager write, retrieval/vector runtime, A2A handoff, dependency upgrade, CI blocking, audit/memory write, or Phase 2 NIM/HPC/Slurm/BCM authority was opened.

## [M5.85-13] - Chinese comments for analysis query helpers

**Delivery**: Completed the fourth Batch 5 Chinese teaching comment slice for analysis/catalog query helpers.

**Changes**
- Added Chinese teaching comments to `VirtualMachineQuerySupport`, explaining VM name path-segment validation, URL encoding, and why VM query support does not open start/stop/delete authority.
- Added Chinese teaching comments to `RepositoryCatalogQuerySupport`, explaining catalog/tag sensitive-read query boundaries and why NIM-related catalog filters are not image pull, deployment creation, NIM creation, or Phase 2 runtime restoration.
- Added Chinese teaching comments to `SaleProductQuerySupport`, explaining product/lease quote query whitelisting and why quote estimation is not order creation, payment, renewal, or quote approval.
- Added Chinese teaching comments to `IndustryAppQuerySupport`, explaining template/instance/API-history read query boundaries, no instance creation, no API invocation, and no history replay.
- Added Chinese teaching comments to `FinancialAnalysisQuerySupport`, explaining cost/billing sensitive-read filters and why query fields cannot express identity, tenant, payment, write, or release state.
- Added `Batch5AnalysisQuerySupportChineseCommentContractTest` and `AnalysisQuerySupportContractTest`.

**Verification**
- `mvn -q "-Dtest=Batch5AnalysisQuerySupportChineseCommentContractTest,AnalysisQuerySupportContractTest,VirtualMachineReadToolHttpContractTest,RepositoryCatalogToolHttpContractTest,SaleProductAnalysisToolHttpContractTest,IndustryAppAnalysisToolHttpContractTest,FinancialAnalysisToolHttpContractTest" test` passed.

**Security**
- Comment/test/documentation slice only; no runtime behavior changed.
- Documents that analysis/catalog helper output remains GET-only query/path filtering and still depends on trusted token/orgId, ToolPermission, HITL for sensitive reads, and kube-manager permissions.
- No Tool execution, MCP runtime, kube-manager runtime call change, LLM/Embedding runtime change, retrieval/vector runtime, audit/memory write, dependency upgrade, CI blocking, order/payment/deployment/NIM runtime authority, or NIM/HPC/Slurm/BCM Phase 2 authority was opened.

## [M5.85-12] - Chinese comments for query path body helpers

**Delivery**: Completed the third Batch 5 Chinese teaching comment slice for kube-manager query/path/body support helpers.

**Changes**
- Added Chinese teaching comments to `DownloadTaskQuerySupport`, `CoursewareQuerySupport`, `TemplateQuerySupport`, `TensorBoardQuerySupport`, and `FileStorageQuerySupport`, explaining resource ID/path/query parameter boundaries before kube-manager HTTP calls.
- Added Chinese teaching comments to `UserRiskMutationSupport`, explaining high-risk user mutation target ID validation, recharge body whitelisting, and why control fields cannot be passed to kube-manager.
- Added `Batch5QuerySupportChineseCommentContractTest` to protect helper teaching markers.
- Added `QuerySupportPathAndBodyContractTest` to verify positive ID extraction, path/query/fragment injection rejection, file/storage required string behavior, recharge body whitelisting, unsafe target rejection, invalid amount rejection, and oversized amount rejection.
- Strengthened `HighRiskMutationToolHttpContractTest` to ensure `writeAllowed` and `releaseDecision` caller fields are not included in recharge body.

**Verification**
- `mvn -q "-Dtest=Batch5QuerySupportChineseCommentContractTest,QuerySupportPathAndBodyContractTest,DownloadTaskProgressToolHttpContractTest,HighRiskMutationToolHttpContractTest" test` passed.

**Security**
- Comment/test/documentation slice only; no runtime behavior changed.
- Documents that resource IDs are locators, not authorization; path/query/body helper output still depends on trusted token/orgId, ToolPermission, HITL, durable audit, release evidence, and kube-manager permissions.
- No Tool execution, MCP runtime, kube-manager runtime call change, LLM/Embedding runtime change, retrieval/vector runtime, audit/memory write, dependency upgrade, CI blocking, or NIM/HPC/Slurm/BCM Phase 2 authority was opened.

## [M5.85-11] - Chinese comments for support contracts

**Delivery**: Completed the second Batch 5 Chinese teaching comment slice for support-contract classes.

**Changes**
- Added Chinese teaching comments to `ToolParameterSpec`, clarifying that parameter schema / aliases describe business input shape only and cannot supply identity, HITL, audit, release, token, orgId, or write authority.
- Added Chinese teaching comments to `SafeToolExecutionResult` and `SafeToolExecutionSource`, separating executed / success / clarification / graph-display semantics from permission and release authority.
- Added Chinese teaching comments to `AgentTraceContext`, explaining traceId as an observability correlation key, not identity, tenant, session, HITL, audit receipt, release evidence, or Tool authorization.
- Added Chinese teaching comments to intent support contracts: `IntentDefinition`, `IntentResult`, `ScoreNormalizer`, `EmbeddingConfig`, and `L3ClassificationResult`.
- Added Chinese teaching comments to default-value support classes and Tool exceptions: `IntentDefaults`, `DefaultValueRegistry`, `DefaultValueApplier`, `AtlasToolValidationException`, and `PermissionDeniedException`.
- Added `Batch5SupportContractBehaviorTest` and expanded `Batch5ChineseCommentContractTest`.

**Verification**
- `mvn -q "-Dtest=Batch5ChineseCommentContractTest,Batch5SupportContractBehaviorTest,AgentTraceContextTest,M521DefaultValueSafetyContractTest,DefaultValueRegistryTest" test` passed.

**Security**
- Comment/test/documentation slice only; no runtime behavior changed.
- Documents that schema, execution result, execution source, traceId, intent classification, default values, validation errors, and permission-denied errors are support contracts, not permission, HITL, audit, release, MCP runtime, kube-manager write, deployment, or Phase 2 authority.
- No Tool execution, MCP runtime, kube-manager call, LLM/Embedding runtime change, retrieval/vector runtime, audit/memory write, dependency upgrade, CI blocking, or NIM/HPC/Slurm/BCM Phase 2 authority was opened.

## [M5.85-10] - Chinese comments for support foundation

**Delivery**: Completed the first Batch 5 Chinese teaching comment slice for DTO / store / config support foundation.

**Changes**
- Added Chinese teaching comments to API and login DTOs: `ApiResponse`, `LoginRequest`, `LoginResponse`, and `SessionData`.
- Added Chinese teaching comments to conversation DTOs: `Conversation`, `ConversationItemDto`, and `ConversationDetailDto`.
- Added Chinese teaching comments to `SessionStore` and `ConversationStore`, clarifying that sessionId is not a JWT, conversationId is not authorization, and owner checks must use trusted server principal/user context.
- Added Chinese teaching comments to `AtlasAsyncConfig`, explaining token/orgId async propagation, old-value restoration, and why request body / LLM / frontend fields cannot derive identity.
- Added Chinese teaching comments to `AtlasConfiguration`, explaining optional Embedding/L3 LLM degradation and why intent classification cannot bypass SafeToolExecutor, HITL, audit, kube-manager permissions, Memory/RAG source custody, or release gates.
- Added `Batch5ChineseCommentContractTest` to protect the support-layer teaching markers.

**Verification**
- `mvn -q "-Dtest=Batch5ChineseCommentContractTest,ConversationStoreTest,ConversationControllerTest,AuthControllerLoginFailSafeTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- Comment/test/documentation slice only; no runtime behavior changed.
- Documents that API success, sessionId, conversationId, login request orgId, intent classification, and async propagation are support contracts, not runtime authority.
- No Tool execution, MCP runtime, kube-manager access, LLM/Embedding runtime change, retrieval/vector runtime, audit/memory writes, dependency upgrade, CI blocking, or NIM/HPC/Slurm/BCM Phase 2 authority was opened.

## [M5.85-9] - Chinese comments for evidence chain

**Delivery**: Completed Chinese teaching comment Batch 4 for the Memory / RAG / Audit / Observability / Replay / Eval evidence chain.

**Changes**
- Added Chinese teaching comments to Memory/RAG digest contracts, explaining source/chunk/evidence digest layering, stable evidence fingerprints, raw marker rejection, and why digest output is not prompt authority.
- Added Chinese teaching comments to audit recorder/factory/JSONL durable sink/query model, explaining durable audit, redacted-only storage, parameter existence summaries, bounded JSONL reverse scan, and fail-closed high-risk prewrite semantics.
- Added Chinese teaching comments to `ObservabilityController`, `AgentReplayTimelineService`, replay DTOs, `AgentEvalReportService`, eval response DTO, reviewed trace evidence, and Memory/RAG readiness/digest contract services.
- Fixed a mojibake replay endpoint comment in `ObservabilityController`.
- Added `Batch4ChineseCommentContractTest` to protect Batch 4 Chinese learning markers and reject common mojibake fragments in touched evidence-chain files.

**Verification**
- `mvn -q "-Dtest=Batch4ChineseCommentContractTest,MemoryControllerTest,ConversationSummaryMemoryStoreTest,MemoryRagSourceEvidenceDigestDeriverTest,AgentAuditRecorderTest,JsonlAgentAuditDurableSinkTest,ObservabilityControllerTest,ObservabilityControllerSecurityContractTest,AgentReplayTimelineServiceTest,AgentEvalReportServiceTest,AgentReviewedEvalTraceEvidenceServiceTest,AgentMemoryRagReadinessServiceTest,AgentMemoryRagSourceEvidenceDigestContractServiceTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.

**Security**
- Comment/test/documentation slice only; no runtime behavior changed.
- Replay/Eval/Audit/Memory-RAG are documented as admin-only or user-isolated read/redacted evidence, not prompt authority, not Tool execution, not MCP runtime, not kube-manager access, not LLM calls, and not release authority.
- No retrieval/vector/reranker/GraphRAG runtime, CI blocking, durable memory runtime, MCP `tools/call`, A2A handoff, kube-manager write authority, dependency upgrade, or NIM/HPC/Slurm/BCM Phase 2 runtime scope was opened.

## [M5.85-8] - Chinese comments for orchestration chain

**Delivery**: Completed Chinese teaching comment Batch 3 for the Agent orchestration chain.

**Changes**
- Added Chinese teaching comments to `AtlasOrchestrator`, explaining SSE entry, trusted runtime identity, supervisorGraph streaming, clarify-vs-HITL semantics, Graph State runtime-object boundaries, and fail-closed identity/conversation checks.
- Added Chinese teaching comments to `AtlasGraphConfig`, explaining StateGraph merge strategies, `BrainDecision.ActionType` conditional routing, `plan_node`, `execute_node`, `react_node`, `tool_call`, delegate subgraphs, ThreadLocal restore, and the SafeToolExecutor double boundary.
- Added Chinese teaching comments to `ReActEngine`, `ReActPromptBuilder`, and `ReActMemory`, explaining Thought -> Action -> Observation, LLM Action JSON as candidate business input, protected parameter filtering, display redaction, short-term ReAct memory, and SafeToolExecutor as the only real Tool outlet.
- Added Chinese teaching comments to `PlanEngine`, explaining that Plan is planning evidence, not execution permission; it does not call Tools, kube-manager, audit, or HITL marker creation.
- Added Chinese teaching comments to Graph bridge/node classes: `AtlasToolCallback`, `DirectAnswerNode`, `SseEmitNode`, and `ToolResultMergeNode`.
- Added `Batch3ChineseCommentContractTest` to protect Batch 3 learning markers.

**Verification**
- `mvn -q "-Dtest=Batch3ChineseCommentContractTest,SupervisorGraphReactRoutingTest,AtlasOrchestratorRuntimeIdentityTest,AtlasOrchestratorJsonTest,ReActEnginePolicyTest,ReActEngineParamMergeTest" test` passed.
- `mvn -q "-DskipTests" validate` passed.
...
## [M5.85-7] - Code review boundary fixes and learning guide

**Delivery**: Reviewed the completed backend code against current documents, fixed high-risk boundary mismatches, corrected stale documentation, and added a project-based top-tier Agent learning guide.

**Changes**
- Hardened HITL confirm/clarify so checkpoint owner, checkpoint orgId, and current principal orgId are validated before consuming confirmToken or deleting pending decisions.
- Stopped trusting login request `organizationId` as a local Session orgId; trusted orgId must come from kube-manager response or token-based lookup.
- Cached trusted orgId in `UserPermissionContext` so Bearer requests can restore token+orgId context and clear stale ThreadLocal orgId.
- Tightened `/api/agent/mcp/**` to admin-only, matching governance/readiness documentation.
- Set high-risk durable audit defaults to fail-closed and updated tests so HITL alone cannot execute high-risk writes without durable audit readiness.
- Blocked Phase 2 NIM/HPC/Slurm/BCM domain tools from Phase 1 MCP manifest export.
- Clarified Memory summary code comments: caller-submitted bounded summaries are not trusted RAG prompt authority.
- Corrected README, roadmap, document governance/index, ADRs, auth/session design, Tool spec, mission memory, and architecture learning map.
- Added `docs/learning/以kube-agent为例的顶级Agent开发学习指南.md`.

**Verification**
- Focused security/governance suite passed:
  `mvn -q "-Dtest=AuthControllerLoginFailSafeTest,UserPermissionContextTest,AuthTokenFilterSecurityContextTest,AgentPrincipalResolverTest,AgentSecurityConfigContractTest,AgentSecurityConfigWebMvcTest,M513HitlFailClosedContractTest,SafeToolExecutorTest,AgentAuditRecorderTest,JsonlAgentAuditDurableSinkTest,AgentAuditTelemetryPublisherTest,M520McpManifestSafetyContractTest,McpGovernanceOverviewServiceTest,M510ArchitectureBoundaryTest" test`.
- Full `mvn -q test` passed.
- `mvn -q "-DskipTests" validate` passed.
- `git diff --check` passed with Windows LF-to-CRLF warnings only.

**Security**
- No MCP `tools/call`, A2A handoff, retrieval/vector/reranker/GraphRAG runtime, CI blocking, dependency upgrade, or Phase 2 NIM/HPC/Slurm/BCM runtime authority is opened.
- kube-manager write authority remains gated by trusted identity/orgId, HITL, durable audit, idempotency, release evidence, and post-write readback.

## [M5.85-6] - Chinese document filenames

**Delivery**: Renamed current human-facing documentation files to Chinese filenames while keeping tool-convention entry files stable.

**Changes**
- Kept `README.md` unchanged because it is the conventional repository entry requested by the user.
- Kept `AGENTS.md` unchanged because Codex uses this exact filename for project rules.
- Renamed the current roadmap, changelog, Tool spec, document index, document governance, project memory, architecture learning map, auth/session design, ADRs, and tech-stack docs to Chinese filenames.
- Updated current documentation links and recovery-memory mirrors to point at the Chinese filenames.
- Preserved historical `codex-memory` snapshot files with their old names where they are recovery evidence rather than current reading entry points.

**Security**
- Documentation filename/link-only slice; no runtime behavior changed.
- Does not change Java code, Tool execution, MCP runtime, kube-manager calls, HITL, permissions, audit, memory, eval, dependencies, or Phase 2 NIM/HPC/Slurm/BCM scope.
...
- Rewrote `开发路线图.md` as the current development order for Chinese comment batches, Orchestrator hardening, kube-manager READ E2E, Eval evidence, Multi-Agent review, and gated advanced-technology adoption.
- Rewrote `docs/文档索引.md` and `docs/文档治理规则.md` to describe the cleaned documentation tree, truth-source order, deletion rules, Phase 2 document policy, and recovery workflow.
- Moved ADR-008 from the old `docs/v3.1` tree to `docs/adr/ADR-008-SpringAIAlibaba选型决策.md`.
...
- Kept current long-lived docs: `docs/项目使命与当前记忆.md`, `docs/顶级Agent架构与技术学习地图.md`, ADRs, `docs/认证会话API设计.md`, `docs/文档治理规则.md`, current Java tech-stack audit, advanced tech-stack roadmap, `README.md`, `开发路线图.md`, `变更日志.md`, `AGENTS.md`, `Tool开发规范.md`, and `codex-memory`.
...
- Refreshed `docs/文档索引.md` from an obsolete M2-era index into a current M5.85 document map.
- Added `docs/文档治理规则.md` with truth-source order, document categories, cleanup rules, and next cleanup recommendations.
...
**Delivery**: Added a then-authenticated, read-only MCP governance overview that explains why the current MCP surface remains manifest-only and what evidence is required before any future `tools/call` runtime. Current implementation has since tightened `/api/agent/mcp/**` to admin-only.
**Changes**
...
- The endpoint was introduced as authenticated, read-only, manifest-only, and does not execute Tools.
- Current correction on 2026-06-12: `/api/agent/mcp/**` has since been tightened to admin-only, so this governance overview is no longer an ordinary authenticated-user surface.
...
**Delivery**: Bridged frontend `X-Session-Id` sessions into Spring Security and moved the first authenticated non-chat Agent surfaces, `/api/agent/memory/**` and `/api/agent/mcp/**`, behind explicit `authenticated()` rules without breaking existing Bearer-token compatibility. Current correction on 2026-06-12: `/api/agent/mcp/**` has since been tightened from authenticated to admin-only.
**Changes**
...
- Registered the session-aware filter from `AgentSecurityConfig` and protected `/api/agent/memory/**` plus `/api/agent/mcp/**` as the first authenticated non-chat endpoints; current implementation keeps memory authenticated and tightens MCP to admin-only.
...
- Added GitHub Actions backend quality workflow and `docs/tech-stack/后端先进技术栈路线图_20260608.md`.
